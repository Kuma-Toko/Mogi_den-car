...
1. appropriatenessScoreは「今の治療方針をこのまま続けた場合、患者の状態が単位時間あたりどれだけ改善/悪化するか」を
   ...
   - 【重要】ルーブリックが示す第一選択の治療（薬剤・用法用量・投与経路など）が過不足なく正しく開始されている場合は、
     それ単独で90点台後半〜100点を与えること。付随的な補助オーダー（検査・経過観察指示・対症療法など）が
     まだ出ていないことや、治療期間がまだ短いことを理由に減点しないこと——それらはルーブリックが具体的に要求している
     場合にのみ減点材料とする。
